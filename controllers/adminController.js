...
exports.movies = catchAsync(async (req, res, next) => {
  ...
  const { meta, data: movies } = await getRequest(
    req,
    `/movies?limit=30&fields=id,landscape,title,rating,description&order=createdAt,desc`
  );
  // console.log(meta);
...
exports.series = catchAsync(async (req, res, next) => {
  const { meta, data: series } = await getRequest(
    req,
    "/series?limit=30&fields=id,landscape,title,rating,description&order=createdAt,desc"
  );
...
exports.games = catchAsync(async (req, res, next) => {
  ...
  const { meta, data: games } = await getRequest(
    req,
    "/games?limit=30&fields=id,landscape,title,rating,description&order=createdAt,desc"
  );
  // console.log(games);
  const ext = {
    title: "games",
...
exports.schedules = catchAsync(async (req, res, next) => {
  const { meta, data: schedules } = await getRequest(
    req,
    "/schedules?order=date&limit=2"
  );
...
exports.suggestions = catchAsync(async (req, res, next) => {
  const { meta, data: suggestions } = await getRequest(
    req,
    "/suggestions?limit=30&order=createdAt,asc"
  );
...
exports.updateschedule = catchAsync(async (req, res, next) => {
  const { data: schedule } = await getRequest(
    req,
    `/schedules/${req.params.id}`
  );
