...
export function renderLoadContent({ response, containerId, type, cardName }) {
  ...
  if (!data.length) {
    redirector.innerHTML = `We couldn't find results for your search. Results in <a href="/movies${query}">movies</a> or in <a href="/games${query}">games</a>`;
  } else {
    redirector.innerHTML = `Find results in <a href="/movies${query}">movies</a> or in <a href="/games${query}">games</a>`;
  }
}
